...
module.exports = {
	show_matchup_by_id: function(req, res) {
		...
		connection.query('SELECT m.id as matchup_id, c1.id as char1_id, c1.name as char1_name, t.id as tip_id, t.content as tip_content, t.vote_count as vote_count, t.created_at as tip_created_date, t.character_id as tip_character_id, c1.image as char1_image, c2.id as char2_id, c2.name as char2_name, c2.image as char2_image, u.alias as tip_user_alias, l.id as like_id, l.user_id as liked_by_id FROM match_ups m JOIN characters c1 on m.character1_id = c1.id JOIN characters c2 on m.character2_id = c2.id LEFT JOIN tips t on m.id = t.match_up_id LEFT JOIN users u on t.user_id = u.id LEFT JOIN likes l on t.id = l.tip_id WHERE m.id = ? ORDER BY vote_count DESC', [matchupId], function(err, rows) {
			if (err){
	...
	show_matchup_by_chars: function(req, res) {
		connection.query('SELECT m.id as matchup_id, c1.id as char1_id, c1.name as char1_name, t.id as tip_id, t.content as tip_content, t.vote_count as vote_count, t.created_at as tip_created_date, t.character_id as tip_character_id, c1.image as char1_image, c2.id as char2_id, c2.name as char2_name, c2.image as char2_image, u.alias as tip_user_alias, l.id as like_id, l.user_id as liked_by_id FROM match_ups m JOIN characters c1 on m.character1_id = c1.id JOIN characters c2 on m.character2_id = c2.id LEFT JOIN tips t on m.id = t.match_up_id LEFT JOIN users u on t.user_id = u.id LEFT JOIN likes l on t.id = l.tip_id WHERE (character1_id = ? AND character2_id = ?) OR (character1_id = ? AND character2_id = ?) ORDER BY vote_count DESC', [req.body.base_char_id, req.body.matchup_char_id, req.body.matchup_char_id, req.body.base_char_id], function(err, rows) {
			if(err) {
